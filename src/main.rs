...
use waytrogen_iced::{
    app_state::AppState,
    cli::{
	cycle_next_wallpaper, delete_image_cache, print_app_version, print_wallpaper_state, restore_wallpapers, set_random_wallpapers, Cli
    },
...
fn main() -> anyhow::Result<()> {
    ...
    if args.restore {
        ...
        match set_random_wallpapers(&mut config_file) {
            Ok(_) => {
		config_file.write_to_config_file()
	    },
            Err(e) => Err(e),
        }
